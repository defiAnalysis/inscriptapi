...
func GetETHInfo(scrollId, next string, session orm.Ormer) string {
	...
	data := `{"filters":[{"type":"activity_types","params":["sale"]},{"type":"collection","params":{"id":"b9Fby7ll5e"}},{"type":"status","params":{"status":"all"}}],"sort":"registrationDate_asc","scrollId":"` + scrollId + `","next":"` + next + `"}`
